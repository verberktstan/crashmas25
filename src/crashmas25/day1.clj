...
(test/deftest parse-rotation-test
  (test/is (= {:rotation/direction :left :rotation/steps 42}
              (parse-rotation "L42"))))
...
(comment
  ;; Test read-input
  (read-input "resources/day1test.txt"))
...
(defn- do-rotation
  ...
  [dial {:rotation/keys [direction steps]}]
  (let [operator (if (= direction :left) - +)
        steps    (mod steps 100)
...
(test/deftest do-rotation-test
  (let [dial {:dial/at 50 :dial/land-on-zero-count 0}]
    (test/are [input output] (= output (do-rotation dial input))
      {:rotation/direction :left :rotation/steps 1}
      (assoc dial :dial/at 49)

      {:rotation/direction :right :rotation/steps 1}
      (assoc dial :dial/at 51)

      {:rotation/direction :left :rotation/steps 52}
      (assoc dial :dial/at 98)

      {:rotation/direction :right :rotation/steps 53}
      (assoc dial :dial/at 3)

      {:rotation/direction :left :rotation/steps 50}
      {:dial/at 0 :dial/land-on-zero-count 1})))

(defn- count-zero-crossings
  [dial {:rotation/keys [direction steps]}]
  (let [operator (if (= direction :left) - +)
        mod-op (comp #(mod % 100) operator)
        coll (rest
              (reductions mod-op
                          (:dial/at dial)
                          (repeat steps 1)))
        zc-count (->> coll (filter zero?) count)]
    (-> dial
  ...
  (letfn [(day1part1 [filename]
          ...
          (day1part2 [filename]
            (-> filename
                ...
                :dial/zc-count))]
    (test/is (-> "resources/day1test.txt"
                 day1part1
                 (= 3)))
    (test/is (-> "resources/day1input.txt"
                 day1part1
                 (= 1011)))
    (test/is (-> "resources/day1test.txt"
                 day1part2
